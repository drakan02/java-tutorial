...
* **Standalone App:** Nó còn được biết đến với tên gọi khác là Destop App hoặc Windows-based App. Một ứng dụng mà chúng ta cần cài đặt trên mỗi thiết bị như media player, antivirus, … AWT và Swing được sử dụng trong Java để tạo các Standalone App.

* **Web App:** Một ứng dụng mà chạy trên Server Side và tạo Dynamic Page, được gọi là Web App. Hiện tại, các công nghệ Servlet, JSP, Struts, JSF, … được sử dụng để tạo Web App trong Java.
...
* **Mobile App:** Đây là loại ứng dụng được tạo cho thiết bị mobile. Hiện tại thì Android và Java ME được sử dụng để tạo loại ứng dụng này.

# Quy trình biên dịch và thông dịch trong Java

- Chương trình Java sử dụng cơ chế kết hợp giữa biên dịch (compilation) và thông dịch (interpretation) để đảm bảo tính độc lập nền tảng. Quá trình này diễn ra qua hai giai đoạn chính:

1. **Giai đoạn Biên dịch (Compile-time)**
- **Công cụ:** Sử dụng trình biên dịch javac.
- **Hoạt động:** Mã nguồn Java (file .java) được dịch thành một mã trung gian gọi là Bytecode (file .class).
- **Đặc điểm:** Bytecode không phải là mã máy của một CPU cụ thể nào, giúp mã Java có thể "viết một lần, chạy mọi nơi" (Write Once, Run Anywhere). 


2. **Giai đoạn Thông dịch (Runtime)**
- **Công cụ:** Máy ảo Java (JVM - Java Virtual Machine).
- **Hoạt động:** Khi chạy chương trình, trình thông dịch bên trong JVM sẽ đọc từng lệnh Bytecode và chuyển đổi chúng thành mã máy (machine code) tương ứng với hệ điều hành đang sử dụng (Windows, Linux, macOS...) để CPU thực thi.
- **Tối ưu hóa (JIT):** Để tăng tốc độ, JVM hiện đại sử dụng trình biên dịch Just-In-Time (JIT). JIT sẽ biên dịch các đoạn Bytecode được sử dụng thường xuyên thành mã máy trực tiếp thay vì thông dịch lại nhiều lần, giúp chương trình chạy nhanh hơn